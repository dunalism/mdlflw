...
class RoleResource extends Resource implements HasShieldPermissions
{
                ...
                Forms\Components\Section::make()
                    ->schema([
                        ...
                        Forms\Components\Select::make(config('permission.column_names.team_foreign_key'))
                            ...
                            ->options(
                                fn (): Arrayable => Utils::getTenantModel()
    ...
    {
        return Utils::isResourceNavigationBadgeEnabled() ? strval(static::getEloquentQuery()->count()) : null;
    }
    ...
    public static function getTabFormComponentForCustomPermissions()
    {
        $options = static::getCustomPermissionOptions();

        $filteredOptions = array_filter(
            $options,
            fn ($label, $key) => ! str_contains($key, '_role'),
            ARRAY_FILTER_USE_BOTH,
        );

        $count = count($filteredOptions);
        ...
        return Forms\Components\Tabs\Tab::make('custom')
            ...
            ->badge($count)
            ->schema([static::getCheckboxListFormComponent(name: 'custom_permissions', options: $filteredOptions)]);
    }
}
